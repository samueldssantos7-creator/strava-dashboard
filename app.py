...
# ATENÇÃO: Verifique se estas funções estão corretamente exportadas em seu etl.py
from etl import (
    renew_access_token,
...
def pace_by_category(df_in):
    ...
    # MUDANÇA: Remove títulos e OCULTA o eixo Y (ticks e labels)
    fig.update_layout(
        xaxis_title=None, 
        yaxis_title=None, 
        xaxis_tickangle=-45,
        yaxis=dict(showgrid=False, zeroline=False, showticklabels=False) # REMOVE EIXO Y
    )
...
if btn_fetch:
    ...
    if csv_path.exists():
        ...
        df = pd.DataFrame()
        st.warning(f"Sem dados! O arquivo esperado em '{csv_path}' não foi encontrado. Pressione 'Buscar/Atualizar dados' ou verifique o seu etl.py.")
...
with col1:
    ...
    # Linha Branca e Bolinhas FC4C02
    fig1.update_traces(
...
with col2:
    ...
    # Cores customizadas, usando FC4C02
    fig2.update_traces(
...
# Aplica a cor FC4C02, arredonda os cantos e remove eixo Y
